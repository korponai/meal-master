...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <header class="bg-white border-b border-gray-200">
      ...
      >
        ...
        <div class="hidden md:flex gap-8 text-sm font-medium text-gray-600">
          ...
          <template v-if="user">
            <NuxtLink
              :to="localePath('/meals/weekly-meal-plan')"
              :class="[
                isActive('/meals/weekly-meal-plan')
                  ? 'text-black'
                  : 'hover:text-black transition-colors',
              ]"
              >{{ $t("meal_plan") }}</NuxtLink
            >
            <NuxtLink
            ...
            >
            <NuxtLink
              :to="localePath('/meals/shoppinglist')"
              :class="[
                isActive('/meals/shoppinglist')
                  ? 'text-black'
                  : 'hover:text-black transition-colors',
              ]"
              >{{ $t("shopping_list") }}</NuxtLink
            >
          </template>
        </div>
    ...
    <footer class="bg-gray-900 text-white py-8 mt-auto">
      <div
        class="max-w-7xl mx-auto px-4 md:px-8 flex flex-col md:flex-row justify-between items-center text-sm text-gray-400"
      >
